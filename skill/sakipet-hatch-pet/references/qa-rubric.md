...
When an `animations.json` sidecar is requested:

- `pet.json` remains unchanged and the V2 atlas still passes all required checks.
- Every clip uses a safe `animations/` path, complete `192x208` frames, a matching duration count, and valid `mode` or `gesture` semantics.
- Clip strips have exact `frames * 192 x 208` dimensions, decode successfully, and include transparency.
- Clip previews show stable identity, scale, baseline, cadence, and loop closure at in-app size.
- Invalid optional clips never replace or invalidate the otherwise valid V2 fallback package.
