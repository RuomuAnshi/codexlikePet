---
name: sakipet-hatch-pet
description: Create, repair, validate, visually QA, and package SakiPet frame-animated pets while preserving the Codex-compatible v2 atlas and unchanged pet.json format. Use for new pets, custom mascot art, v2 repairs, 8x11 spritesheets, or optional sidecar animations.json frame clips.
---
...
Create a SakiPet-compatible, Codex V2 animated pet from a concept, brand cue, company/prospect name, one or more reference images, or any combination of those inputs. Every newly hatched pet keeps the unchanged `pet.json` shape, includes an 8x11 atlas with the 9 standard animation rows plus 16 clockwise look directions, and is packaged with `spriteVersionNumber: 2`. Optional full-body frame clips are stored in an adjacent `animations.json` sidecar and never added as extra rows to the V2 atlas. The intermediate 8x9 atlas exists only to assemble and review rows 0-8; never package it as a new pet.

For the optional SakiPet frame-animation extension, read [references/frame-animation-pack.md](references/frame-animation-pack.md). Use it when the user asks for additional actions beyond the fixed V2 rows. The extension preserves `pet.json` byte-for-byte, uses 192x208 full-body frames, and falls back to the base V2 pet when the sidecar is absent or invalid.
...
```bash
SKILL_DIR="${CODEX_HOME:-$HOME/.codex}/skills/sakipet-hatch-pet"
...
## Optional SakiPet Frame-Animation Stage

When the user requests actions beyond the fixed V2 rows, complete the required V2 pet first, then add optional full-body frame clips. Do not add rows to `spritesheet.webp`, change `pet.json`, or replace the Codex contract. Read [references/frame-animation-pack.md](references/frame-animation-pack.md) before preparing the extension.

Use one visual worker per requested clip. Each worker must generate one coherent horizontal strip grounded by the canonical base and approved V2 contact sheet. Suggested first-pass clips for a cat-like pet are `sleep`, `wake`, `yawn`, `ear-twitch`, `tail-swish`, `groom`, `startled`, `celebrate`, `annoyed`, and `stretch`. Choose only clips that add a distinct readable verb for the pet.

For every clip:

1. Write the clip's mechanics before generation: stable anchor, moving parts, entry, main pose, exit, loop behavior, and interruption behavior.
2. Generate a complete strip with `$imagegen`; do not locally draw, tile, rotate, warp, recolor, or patch frames. Use the canonical base and the V2 contact sheet as identity references.
3. Normalize the selected source strip with `scripts/assemble_frame_clip.py`, using the source's frame count and the run's chroma key. This deterministically removes the key, extracts separated poses, keeps a shared baseline, writes `frames * 192` by `208` pixels with alpha, and can emit a GIF preview. For a persistent mode such as sleep, reserve the leading frames for entry and set `loopStart` to the first stable repeating frame. Copy the normalized result to `animations/<clip-id>.webp` in the package.
4. Add the clip's timing and semantics to `animations.json` and run:

```bash
"$PYTHON" "$SKILL_DIR/scripts/validate_animation_pack.py" \
  /absolute/path/to/pet-directory \
  --json-out /absolute/path/to/qa/animation-pack-validation.json
```

5. Render and inspect each clip at in-app size. Reject scale pops, baseline jumps, missing entry/exit phases, identity drift, clipping, interior transparent holes, detached effects, or effectively static loops.

The final package still contains the original `pet.json` and V2 `spritesheet.webp` unchanged, with `animations.json` and `animations/` added beside them. If the optional pack is absent or fails validation, the runtime must fall back to the V2 rows.
