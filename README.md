...
打招呼、交头接耳、贴贴、追逐、捉人、堆叠、抢玩具、分享零食、安慰、踢足球和一起睡觉；
三到四只宠物还支持队列、围圈聊天、群体堆叠和群体欢呼。足球比赛由真实的物理模拟驱动：
球有速度、滚动摩擦和边界反弹，每只宠物拥有碰撞体积，最近的一只上前抢球、第二近的
拦截球路、其余的包抄。触球会按情境分类：慢速触碰把球轻推向前形成运球盘带，连续
几次触球后蓄力射门，其他宠物碰到球则是抢断解围，两到四只宠物的比赛过程完全涌现
而不是编排好的。场景会避开正在使用的
舞台，用户拖拽、隐藏、暂停、睡眠或开启安静模式时会立即取消，冲突的场景会自动排队重试。

应用内置足球、零食、毛绒玩具和彩带道具，不需要用户下载或安装。足球支持踢出、滚动、
弹跳、追球和轮流传递；零食支持发现、分享和争抢；毛绒玩具支持传递、抢夺、拉扯、合作
和竞争。道具窗口只负责绘制贴图，Rust 社交运行时负责道具轨迹、宠物碰撞和场景安全。
资源说明和元数据位于 `public/props/`。
...
当前内置的 `sleep`、`wake`、`yawn`、`stretch` 会按每只宠物自己的资源自动适配：入睡时优先播放
`yawn` 再进入 `sleep` 循环，醒来后播放 `wake` 和可选的 `stretch`；缺少任一 clip 时只回退缺少的部分，
不会影响其他动作或其他宠物。
拖拽抛掷同样支持扩展 clip：`fall`（建议 `loop: true`）在宠物被扔出后的空中循环播放，缺失时
回退为 running 扑腾；`land`（`loop: false`）在落定弹跳后播放一次，缺失时仅有缩放挤压效果。
两种 clip 均为可选，不影响未提供它们的宠物包。
